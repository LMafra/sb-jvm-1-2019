  ...
  for (int i = 0; i < 1; i++) {
    ...
    if (!strcmp((char*)cf->constant_pool[cp_index].info.Utf8.bytes, "Deprecated")) {
      ...
      printf("\t\tInnerClasses :\n");
        ...
        printf("\t\t\tinner_class_info_index: #%d ", cf->attributes[i].att_info.InnerClasses.classes_array[j].inner_class_info_index);
        cpIndexReader(cf->constant_pool, cf->attributes[i].att_info.InnerClasses.classes_array[j].inner_class_info_index);
        printf("\n");
        printf("\t\t\touter_class_info_index: #%d ", cf->attributes[i].att_info.InnerClasses.classes_array[j].outer_class_info_index);
        cpIndexReader(cf->constant_pool, cf->attributes[i].att_info.InnerClasses.classes_array[j].outer_class_info_index);
        printf("\n");
        printf("\t\t\tinner_name_index: #%d ", cf->attributes[i].att_info.InnerClasses.classes_array[j].inner_name_index);
        cpIndexReader(cf->constant_pool, cf->attributes[i].att_info.InnerClasses.classes_array[j].inner_name_index);
        printf("\n");
        printf("\t\t\tinner_class_access_flags: %d ", cf->attributes[i].att_info.InnerClasses.classes_array[j].inner_class_access_flags);
